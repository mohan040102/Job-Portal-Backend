 ...
 */
function transaction(req: Request, res: Response, next: NextFunction) {
    ...
    const startText = color.green(`START:${getProcessingTimeInMS(start)}`)
    const idText = color.blue(`[${id}]`);
    const timeStampText = color.blueBright(`[${timestamp}]`);
    ...
    res.once('finish', () => {
        ...
        const endText = color.red(`END:${getProcessingTimeInMS(end)}`);
